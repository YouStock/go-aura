...
#
# The cluster contains a bootnode, a aura node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the aura and
# swarm nodes
start_bootnode() {
...
# start_aura_node starts a aura node with --datadir pointing at <base-dir>/aura
# and a single, unlocked account with password "aura"
start_aura_node() {
  local dir="${base_dir}/aura"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get aura account address"
  fi
  ...
  start_node "aura" "${GETH_IP}" "$(which aura)" ${args[@]}
}
...
create_account() {
  ...
  aura --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
