Name "aura ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install aura binary
Section "Aura" GETH_IDX
  SetOutPath $INSTDIR
  file {{.Aura}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\aura.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\aura.exe" "attach" "" ""
  ...
  # Firewall - add rules
  SimpleFC::AdvAddRule "Aura incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\aura.exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "Aura outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\aura.exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "Aura UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\aura.exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\aura.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\aura.ipc"
